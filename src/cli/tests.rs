...
use clap::CommandFactory;
    ...
    fn doctor_help_excludes_serp_flags() {
        ...
        assert!(
            lines <= 90,
            "doctor help too long ({lines} lines); SERP pollution?"
        );
        for ban in ["--vertical", "--shared-session-verticals", "--num ", " -n,", "--pages"] {
            assert!(
                !help.contains(ban),
...
{help}"
    ...
    fn print_budget_without_query_and_agent_flags_parse() {
        ...
        assert_eq!(r2.buscar.fields.as_deref(), Some("url,titulo"));
        assert_eq!(r2.buscar.result_filter.as_deref(), Some("host:example.com"));
    }
    ...
    fn output_global_aceito_apos_subcomando() {
        let r = RootArgs::try_parse_from(["bin", "deep-research", "rust", "-o", "/tmp/x.json"])
            .expect("-o deve ser aceito apos deep-research (V13 deep knobs)");
        match r.subcommand {
            Some(Subcommand::DeepResearch(dr)) => {
                assert_eq!(
