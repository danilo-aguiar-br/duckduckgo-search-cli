...
use crate::zero_cause_is_non_legitimate;
    ...
    fn build_config_accepts_multi_query_with_news_vertical() {
        ...
        let cfg = crate::runtime::build_config(&args).expect("multi-query + --vertical news must be accepted");
    ...
    fn build_config_auto_paginates_when_num_above_10() {
        ...
        for (num, expected_pages) in casos {
            ...
            args.pages = 1;
            let cfg =
                crate::runtime::build_config(&args).unwrap_or_else(|e| panic!("should build for num={num}: {e}"));
            assert_eq!(
