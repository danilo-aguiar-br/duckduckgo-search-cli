...
pub(crate) fn is_quality_depth_subquery(parent: &str, candidate: &str) -> bool {
    ...
    let has_new_content = cand_content
        .iter()
        .any(|t| !parent_set.contains(t.as_str()));
    if !has_new_content {
        return false;
    }
    true
...
pub(crate) fn heuristic_depth_follow_ups(
    ...
    let stop = depth_stopwords();
    let parent_content: std::collections::HashSet<String> =
        content_tokens(&tokenize_depth_query(original), &stop)
            .into_iter()
            .collect();

    let mut freq: HashMap<String, usize> = HashMap::with_capacity(64);
    for item in aggregated.iter().take(12) {
        let blob = format!("{} {}", item.title, item.snippet.as_deref().unwrap_or(""));
