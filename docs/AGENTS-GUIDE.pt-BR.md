...
- v1.0.2: wire JSON em inglês por padrão (ADR-0027) + `--wire-keys en|pt`; agent ops (`--fields`/`--filter`/`--limit`/…); mute-audio (ADR-0026); budget dual/contenção (ADR-0024/0025); `budget_profile`; defaults deep max-sub 3 / fetch-cap 4
- Defaults agent-ready (v0.9.8, GAP-WS-AGENT-READY-001 / ADR-0018): padrão `--vertical all` (web + notícias), fetch de conteúdo LIGADO nas top web + news (teto 4 (padrão v1.0.2); opt-out `--no-fetch-content`), Chrome multi-canal (export Flatpak → ELF de deploy), metadados agent honestos `chrome_path_resolved` / `chrome_channel` / `used_chrome` (não telemetria), flags de transporte globais incluindo `--chrome-path`
...
- Chaves wire JSON em inglês por padrão (v1.0.2 / ADR-0027): `results`, `title`, `metadata`, `result_count`, `chrome_channel`, … Emit legado PT: `--wire-keys pt`
### Tratamento de Erros
...
## Comandos (inventário completo v1.0.6)
- Busca root — `timeout 180 duckduckgo-search-cli "QUERY" -q -f json`
- (oculto) `buscar` — equivalente à busca root — `duckduckgo-search-cli buscar "QUERY" -q -f json -n 5`
...
- `doctor [--strict|--probe-deep]` — `timeout 30 duckduckgo-search-cli doctor -q` (não `doctor --probe`)
- `locale` — `duckduckgo-search-cli locale -q`
- `man [--file PATH]` — `duckduckgo-search-cli man`
- `config path|list|get|set|unset|effective` — `duckduckgo-search-cli config list` (o subcomando é `config list`, NUNCA `config list-keys`)
...
- Root `--probe` — `timeout 15 duckduckgo-search-cli --probe -q -f json` (separado de `doctor --probe-deep`)
...
- Root `--no-input` — `duckduckgo-search-cli "QUERY" -q -f json --no-input` (contrato de agente: nunca pergunta e nunca lê TTY interativo; lida desde a v1.0.6, GAP-REL-008)
...
- `.results[].title` — SEMPRE presente quando `results` não está vazio
- `.results[].url` — SEMPRE presente quando `results` não está vazio
- `.results[].position` — SEMPRE presente quando `results` não está vazio
- `.results[].snippet` — opcional (`Option<String>`) — SEMPRE use fallback `// ""`
- `.results[].display_url` — opcional — SEMPRE use fallback `// .url`
- `.metadata.used_fallback_endpoint` — `true` indica degradação de reputação do IP
- Campos de conteúdo `.content` e `.content_size` — comuns nos top resultados com fetch ligado (padrão LIGADO desde a v0.9.8, teto 4 (padrão v1.0.2)); ausentes com `--no-fetch-content`
- Metadados agent (não telemetria): `.metadata.chrome_path_resolved`, `.metadata.chrome_channel`, `.metadata.used_chrome` honesto
### Modo Deep-Research (v0.8.7+)
- JSON do deep-research usa os MESMOS nomes de campo da busca normal: `.results[].title` (não `.title`)
...
# Host sem Chrome: instale Chrome/Chromium (e Xvfb em Linux headless). Chrome ausente → exit 2 fail-closed.
```
...
- Resultados de notícias ficam em `.news[]` — campos garantidos: `position`, `title`, `url`; campos opcionais (use fallback `// ""`): `source`, `relative_date`, `thumbnail`; com fetch padrão LIGADO também podem incluir `content`
- `.news_count` e `.metadata.vertical_used` costumam estar presentes no padrão `all`
- `--pre-flight` aplica-se SOMENTE à vertical web — é pulado com `--vertical news`
- Zero artigos em SERP de notícias renderizada classifica como `zero_cause: vertical-no-results` — zero legítimo (exit 5, NÃO 6)
...
- Raiz de multi-query é `{ query_count, searches: [...] }` — NUNCA `.results` na raiz
- Raiz de consulta única é `{ query, results, metadata }` — sem chave `.searches`
- SEMPRE distinga a estrutura raiz antes de acessar os resultados

```bash
# consulta única
duckduckgo-search-cli -q -f json "uma" | jaq '.results | length'
# multi-query
duckduckgo-search-cli -q -f json "uma" "duas" | jaq '.searches[0].results | length'
```
...
| 5 | Zero resultados (inclui `vertical-no-results` com `--vertical news`, v0.8.9) | Amplie a consulta; tente `--lang` ou `--country` diferentes |
| 6 | Bloqueio suspeito (zero_cause != legitimo) | Inspecionar `.metadata.zero_cause`; usar `--pre-flight` |
| 130 | Cancelado por SIGINT / Ctrl+C (`128+2`; cancel cooperativo) | Cancelamento pedido pelo agente — não é falha de busca; re-execute quando quiser |
...
- Proxy é somente CLI `--proxy` / `--no-proxy` + XDG `config set proxy_url` — `HTTP_PROXY` / `HTTPS_PROXY` não são lidas
- Prefira XDG a segredos de longa duração em argv
...
- Inspecione `metadata.identity_used` (ex. `chrome-linux-11111111aaaa0001`) para saber qual identidade teve sucesso
- Inspecione `metadata.cascade_level` (0..=4) para saber o quão esgotado o pool está
- Use `--probe` para verificações de saúde de pré-voo LOCAIS antes de lançar queries reais (o projeto proíbe CI/CD)
...
SAIDA=$?
if [ $SAIDA -eq 0 ]; then
  echo "$RESULTADOS" | jaq -r '.results[] | "## \(.title)\n\(.snippet // "")\nURL: \(.url)\n"'
fi
```
...
[ $? -eq 0 ] && jaq -r '.results[].url' /tmp/busca.json
```
...
jaq -r '.searches[].results[].url' /tmp/resultados.json | sort -u
```
...
- Fetch de conteúdo multiplica a latência (top web + news, teto 4 (padrão v1.0.2)); opt-out com `--no-fetch-content`
...
- Use `duckduckgo-search-cli --probe` como portão de pré-voo LOCAL antes de lançar queries reais (v0.6.5+)
- Trate `.metadata.identity_used` como `Option<String>` — use `// "n/a"` como fallback no `jaq` (v0.6.5+)
- Trate `.metadata.cascade_level` como `Option<u32>` — use `// 0` como fallback no `jaq` (v0.6.5+)
- Para testes reproduzíveis use `--identity-profile <nome>` em vez de apenas `--seed` (v0.6.5+)
- Vertical padrão é `all` (v0.9.8) — use `--vertical news` para grounding só de notícias e parseie `.news[]` com fallbacks `// ""` em `source`, `relative_date`, `thumbnail`
- Leia `.metadata.chrome_path_resolved` / `.metadata.chrome_channel` como metadados agent opcionais (não telemetria)
- Sem Chrome, a produção falha fechada com exit 2 (v0.9.4, GAP-WS-113) — não espere auto-degradação web-only
- Parseie `.news[]` (SEMPRE presente no envelope do deep-research, vazio sob `--no-news`) para artigos frescos quando o Chrome está disponível (v0.8.9, GAP-WS-105)
...
- Permissões Unix são `0o600` (owner read+write only)
- O arquivo é estado interno e NÃO é exposto no schema JSON de saída


## v0.7.4 — Preflight NASM no Windows (apenas build-time)
- v0.7.4 adiciona um preflight no build.rs que detecta nasm.exe no PATH em builds nativos Windows MSVC
- Sem o NASM, o build falha em segundos com a correção exata em vez de minutos adentro de erros crípticos do CMake
- O preflight é apenas build-time; sem novas flags CLI, sem novos campos JSON
- Env var HISTÓRICA, morta desde a v0.8.6: DDG_SKIP_NASM_CHECK=1 pulava o preflight em ambientes de build customizados
- O preflight foi removido e a variável NÃO é lida; env var nunca é config de produto
...
- v0.7.5 estende o preflight da v0.7.4 para todas as quatro ferramentas que o build do BoringSSL precisa no Windows MSVC: NASM, CMake 3.20+ (com o sub-componente C++ CMake tools for Windows), MSVC C/C++ toolchain (cl.exe/link.exe) e Strawberry Perl
- Cada ferramenta ausente dispara panic em segundos com a correção exata
- Env vars HISTÓRICAS, mortas desde a v0.8.6: DDG_SKIP_CMAKE_CHECK=1, DDG_SKIP_MSVC_CHECK=1, DDG_SKIP_PERL_CHECK=1 (mais DDG_SKIP_NASM_CHECK=1 da v0.7.4)
- O preflight BoringSSL foi removido e nenhuma delas é lida; env var nunca é config de produto
- Scripts auxiliares HISTÓRICOS em scripts/ (só de mantenedor; mortos desde a v0.8.6, nunca fazem parte do fluxo de agente):
  - install-windows.ps1 — auto-instalava NASM, CMake, Perl; reportava MSVC com instrução Launch-VsDevShell.ps1
...
- Zero impacto em runtime — mesmas flags, mesmo schema JSON de saída, mesmas dependências da v0.7.4
- O crates.io NÃO distribui binários pré-compilados para nenhuma plataforma
- Contagem de testes MEDIDA NA v0.7.5 e nunca remedida desde então: 405 testes lib (eram 392 no total v0.7.0; 333 na v0.6.5 histórica) — a contagem da v1.0.6 é maior e não está declarada aqui


## v0.7.6 — Correção do lockfile do cargo install (GAP-WS-48, apenas build-time)
- v0.7.6 fecha a colisão GAP-WS-48 entre `alloc-no-stdlib 2.0.4` e `3.0.0` no `cargo install` removendo a dep `wreq-util` e a feature `brotli` do `wreq`
- Três pins no `Cargo.toml` mantêm a supply chain determinística: `brotli-decompressor = "=5.0.1"`, `alloc-no-stdlib = "=2.0.4"` (adicionado na v0.7.7) e a escolha de `wreq 6.0.0-rc.29`
- Zero novas flags CLI, zero novos campos JSON, zero mudanças de schema
- `cargo install duckduckgo-search-cli --locked` ERA o caminho suportado em sistema novo naquela release; hoje instale `cargo install duckduckgo-search-cli --locked --version 1.0.6`
...
- v0.7.7 restaura o fingerprint JA4_o que vence o interstitial anti-bot do DDG
- A correção re-adiciona `wreq-util 3.0.0-rc.12` com `default-features = false` e `features = ["emulation"]`, mais os três pins diretos documentados na v0.7.6
- O gap da v0.7.6 era que `--probe-deep` retornava `status: "ok"` enquanto queries reais voltavam zero resultados
- Zero novas flags CLI, zero novos campos JSON
- `cargo install duckduckgo-search-cli --version 0.7.7 --locked` ERA o caminho recomendado naquela release; hoje instale `cargo install duckduckgo-search-cli --locked --version 1.0.6`
- `cargo tree` deve mostrar `wreq-util 3.0.0-rc.12`, `brotli 8.0.3`, `brotli-decompressor 5.0.1`, `alloc-no-stdlib 2.0.4`
- Smoke test de query real: `duckduckgo-search-cli "rust async runtime" -q -f json` deve retornar `result_count >= 5`


## v0.7.8 — Renovação do detector anti-bot + endurecimento de UX (GAP-WS-50..57)
- v0.7.8 fecha 8 gaps funcionais em um único release
- O contrato de schema fica inalterado (zero breaking changes), mas várias flags CLI e comportamentos internos foram endurecidos
...
- A query de calibração do probe-deep agora é o pangrama de 9 palavras `the quick brown fox jumps over the lazy dog` (constante `PROBE_CALIBRATION_QUERY` em `src/probe.rs`)
- A query de 1 palavra `rust` retornava a home page do DDG sem acionar o detector, gerando falso negativo
- `--allow-lite-fallback` historicamente consultava o detector antes de cair para `lite` (v0.7.8–v0.9.3)
- Desde a v0.9.4 (GAP-WS-113) a flag é no-op no modo de produção Chrome-only
...
- O subcomando `buscar` agora é `#[command(hide = true)]`
- Continua invocável mas não aparece no `--help`
- `--retries N` agora é honrado de ponta a ponta em `src/search/retry.rs::execute_with_retry`
- O bug pré-v0.7.8 deixava o valor hard-coded em 1, ignorando a flag
- A faixa aceita é `0..=10`, medida no `--help` da v1.0.6, para impedir `--retries 999` de acionar anti-bot
- 1 teste de regressão em `tests/integration_search_retry.rs` valida que `--retries 5` produz `metadata.retries == 5` no JSON (wire EN; legado PT via `--wire-keys pt`)
