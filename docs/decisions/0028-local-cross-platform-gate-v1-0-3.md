...
- Status: Accepted (2026-08-07), partially superseded by ADR-0029 (2026-08-08)
...
> SUPERSEDED BY ADR-0029 — do NOT follow this recipe. Everything from here to
> the end of this section describes a `zig` / `cargo-zigbuild` shim built on a
> Python venv and a shell wrapper. ADR-0029 removed the dependency entirely
> instead of provisioning it, so none of this is needed and none of it is run.
> It is kept as the record of a decision that was tried and reversed, and the
> full correction is stated below the recipe. This banner sits ABOVE the block
> because a reader who stops at the first code fence would otherwise install a
> Python toolchain that the project no longer uses and explicitly avoids.
