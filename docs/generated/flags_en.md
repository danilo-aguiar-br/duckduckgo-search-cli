...
- This file is NOT generated from any script: the tables were checked BY HAND against binary v1.0.6
- `scripts/regen_cli_flags_readme.py` was deleted in v1.0.4, and nothing regenerates this file today
- The inventory covers the 66 root flags declared in the option column of `duckduckgo-search-cli --help`
- It also covers the `deep-research`, `doctor`, `init-config`, `schema` and `man` exclusives from each subcommand `--help`
- It also covers the hidden aliases `--region` and `--max-concurrency`, which clap accepts but never prints
- The only automated guard is `generated_flag_tables_have_no_phantom_flags` in `tests/integration_docs_drift.rs`
- That ruler feeds every long flag of these tables to clap, so it catches an INVENTED flag
- That ruler NEVER catches a MISSING flag, because it only reads the tokens the tables already carry
- Prefer `commands` / `schema` for low-token agent discovery
- Portuguese prose lives ONLY in [`README.pt-BR.md`](README.pt-BR.md) — this file is English-only
