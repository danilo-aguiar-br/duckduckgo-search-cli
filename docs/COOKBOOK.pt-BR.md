...
| 06 | Contexto longo para RAG/LLM | `duckduckgo-search-cli` (fetch padrão LIGADO desde 0.9.8; `--fetch-content` opcional), `jaq`, `bat`, `timeout` |
...
| 14 | Pipeline busca para sumarização com LLM | `duckduckgo-search-cli` (fetch padrão LIGADO desde 0.9.8; `--fetch-content` opcional), `jaq`, `xh`, `timeout` |
...
| 20 | Circuit breaker per-host em crawl longo (v0.6.5) | `duckduckgo-search-cli` (fetch padrão LIGADO desde 0.9.8; `--fetch-content` opcional), `jaq`, `timeout` |
...
- Benefício: portões locais falham alto no macOS quando bloqueado.
...
```bash
# Persista filtro de produto via XDG (não RUST_LOG)
duckduckgo-search-cli config set log_directive "duckduckgo_search_cli=debug"

# Rode em debug (-v). Omita -q ao capturar logs (-q força off).
duckduckgo-search-cli -v "rust async" -f json --num 5 2> /tmp/ddg.log
# Espere linhas DEBUG/INFO no stderr de duckduckgo_search_cli::*
...
# --allow-lite-fallback é no-op legado (v0.9.4 / GAP-WS-113); manter só por compatibilidade de argv
duckduckgo-search-cli "rust" -q -f json --retries 3 --allow-lite-fallback --num 5
# Espere: caminho SERP HTML via Chrome; a flag não força Lite
```
